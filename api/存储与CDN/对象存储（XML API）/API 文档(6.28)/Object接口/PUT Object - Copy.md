...
在复制过程中，可以指定元数据的处理方式，默认情况下元数据将被复制到目标对象中，也可以选择不复制源对象的元数据信息而在此接口请求中指定新的元数据信息。但是，除非在此接口请求中显式指定存储类型、访问控制列表（ACL）和服务端加密（SSE），否则无论何种处理方式，目标对象均保持为标准存储，继承（default）目标存储桶的 ACL 且不会使用 SSE。
...
- 如果目标对存储桶启用版本控制，对象存储将自动为目标对象生成唯一的版本 ID。
