...
| BUCKET           | 必选参数，指定的存储桶名称，存储桶的命名格式为 BucketName-APPID，参考 [创建存储桶](https://cloud.tencent.com/doc/product/436/6232) | 字符串 |
...
```
 ...
 - 使用`-t tier`具体复原过程类型，枚举值： Expedited （快速模式） ，Standard （标准模式），Bulk（批量模式）；默认值：Standard。
