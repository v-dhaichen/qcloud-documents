...
清单是一种帮助用户管理存储桶中对象的功能，可以有计划地取代对象存储同步 List API 操作。对象存储 COS 可根据用户的清单任务配置，每天或者每周定时扫描用户存储桶内指定的对象或拥有共享前缀的对象（即，其名称前缀为相同字符串的对象），并输出一份清单报告，以 CSV 格式的文件存储到用户指定的存储桶中。文件中会列出存储的对象及其对应的元数据，并根据用户的配置信息，记录用户所需的对象属性信息。
...
为了保证 COS 能将清单报告存储到目标存储桶中，您需要为目标存储桶添加存储桶策略（Bucket Policy），允许COS将对象写入（PUT）目标存储桶。
相关策略如下：
