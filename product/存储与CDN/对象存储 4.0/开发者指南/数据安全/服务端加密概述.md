...
## 加密方式
COS 支持多种服务端加密方式：SSE-COS、SSE-C、SSE-KMS。用户可以自行选择合适的加密方式对存放到 COS 中的数据进行加密。
...
#### 使用对象存储控制台
用户可以参阅 [设置对象加密](https://cloud.tencent.com/document/product/436/33366) 控制台文档，了解如何通过控制台对对象进行SSE-COS加密。
...
SSE-KMS：即使用KMS托管密钥的服务端加密。KMS是腾讯云推出的一款安全管理类服务，使用经过第三方认证的硬件安全模块 HSM（Hardware Security Module） 来生成和保护密钥。帮助用户轻松创建和管理密钥，满足用户多应用多业务的密钥管理需求，符合监管和合规要求。

首次使用SSE-KMS加密，需要[开通KMS服务](https://buy.cloud.tencent.com/kms)，开通KMS服务后，系统会自动为您创建一个默认主密钥（CMK），您也可以通过[KMS控制台](https://console.cloud.tencent.com/kms2)自主从创建密钥，定义密钥策略及使用方法，KMS支持用户自主创建密钥材料来源为**KMS**或**外部**，更多信息请参阅[创建密钥](https://cloud.tencent.com/document/product/573/8875)、[外部密钥导入](https://cloud.tencent.com/document/product/573/38494)。

> 注意
>
> - SSE-KMS 仅加密对象数据，不会加密任何对象元数据。
> - 目前SSE-KMS仅支持北京、上海、广州地域
> - 使用SSE-KMS加密，会产生额外费用，详情请参考[KMS计费概述](https://cloud.tencent.com/document/product/573/34388)。

#### 使用对象存储控制台

用户可以参阅 [设置对象加密](https://cloud.tencent.com/document/product/436/33366) 控制台文档，了解如何通过控制台对对象进行SSE-KMS加密。
...
当用户请求以下接口时，可以通过提供 `x-cos-server-side-encryption` 头部来应用服务端加密，详情请参阅 [公共请求头部 - SSE-KMS](https://cloud.tencent.com/document/product/436/7728#sse-kms)。以下操作支持此头部：
